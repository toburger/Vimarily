...
let shiftKeyToggle = false;
let hudDuration = 5000;
let extensionCommunicator = SafariExtensionCommunicator(messageHandler);
...
function goToFirstInput() {
	...
	const inputToFocus = bestInViewInput || bestInput;
...
Mousetrap.prototype.stopCallback = function (e, element, combo) {
	// Escape key is special, no need to stop. Vimarily-specific.
	...
	if (topWindow) {
		...
		for (let actionName in actionMap) {
			...
			if (actionMap.hasOwnProperty(actionName)) {
				const keyCode = getKeyCode(actionName);
	...
	return function (e) {
		...
		if (settings.transparentBindings === true) {
			if (boundKeys().has(e.key) && !isActiveElementEditable()) {
				...
				// key is bound to a Vimarily action.
				e.stopPropagation();
			}
...
function getKeyCode(actionName) {
	...
	const keyCode = settings['bindings'][actionName];
	const addModifier = (s) => {
 ...
 * This function is required by vimium but deprecated for vimarily as the
 * css is preloaded into the page.
 */
